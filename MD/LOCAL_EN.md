...
10. Process all text data.  
   If you choose to add auxiliary data, run `python training/preprocess.py --add_auxiliary_data True --languages "{PRETRAINED_MODEL}"`  
   If not, run `python training/preprocess.py --languages "{PRETRAINED_MODEL}"`  
   Do replace `"{PRETRAINED_MODEL}"` with one of `{CJ, CJE, C}` according to your previous model choice.

11. Start Training.  
   Run `python training/finetune_speaker.py -m ./OUTPUT_MODEL --max_epochs "{Maximum_epochs}" --drop_speaker_embed True`  
